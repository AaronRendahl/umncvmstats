...
vignette: >
  %\VignetteIndexEntry{Getting Started with R, Rstudio, and Quarto}
...
We'll often want to save the result of a assignment. Let's have it save the result of this computation in the variable `a`. Note that it doesn't output anything when a result is saved, to see the result you have to input the variable again. Think of the `<-` as an arrow pointing to the left.
...
We'll also use functions by using the "pipe" operator, `|>`. Think of it as a triangle that sends the thing on the left into the function on the right. Note that we still need the parenthesis. This makes it easy to chain results of functions together into new functions.
